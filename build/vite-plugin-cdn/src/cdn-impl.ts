...
const parserModuleImpl = (modules: TrackModule[], preset: PresetDomain) => {
  ...
  modules.forEach((module, i) => {
    ...
    try {
      const { version, unpkg, jsdelivr } = tryRequireModule<{
        version: string
        unpkg?: string
        jsdelivr?: string
      }>(`${name}/package.json`)
      switch (preset) {
        ...
        case 'jsdelivr':
          const track = {
            name,
            global,
            spare: [monitor(preset, { jsdelivr, unpkg, version, name }, true)]
          }
          if (spare?.length) {
        ...
        switch (err.code) {
          ...
          case ERRORS.ERR_PACKAGE_PATH_NOT_EXPORTED:
            ...
            if (link === ERRORS.NO_PRESET_FIELDS)
              return bucket.push({ name, type: ERRORS.NO_PRESET_FIELDS })
